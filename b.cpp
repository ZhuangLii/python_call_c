...
int LMICamera::init(char* str){
    ...
    kIpAddress_Parse(&ipAddress, str);
...
extern "C"{
	LMICamera* lmi_new() {return new LMICamera();}
	int lmi_init(LMICamera* lmi, char* str) { return lmi->init(str);} 
	int lmi_grap(LMICamera* lmi) {return lmi->grap();}
	int lmi_del(LMICamera* lmi) {
		int res = lmi->del();
		if (lmi){
			delete lmi;
			lmi = nullptr;	
